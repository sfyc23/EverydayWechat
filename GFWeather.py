...
class gfweather:
    ...
    def __init__(self):
        self.girlfriend_list, self.alarm_hour, self.alarm_minute, self.alarm_second, self.dictum_channel = self.get_init_data()
    ...
    def get_init_data(self):
        ...
        hour, minute, second = [int(x) for x in alarm_timed.split(':')]

        # 将时间转化为上海时间, 第一次部署到服务器时间给搞错了……
        temp_time = datetime(2019, 6, 8, hour, minute, second).astimezone(timezone("Asia/Shanghai")) 
        return girlfriend_list, temp_time.hour, temp_time.minute, temp_time.second, dictum_channel
    ...
    def is_online(self, auto_login=False):
        ...
        for _ in range(5):
            # 命令行显示登录二维码
            itchat.auto_login(enableCmdQR=2) # 修改了Mac命令行二维码显示问题
            if online():
                print('登录成功')
                return True
        else:
    ...
    def run(self):
        ...
        scheduler.add_job(self.start_today_info, 'cron', hour=self.alarm_hour, minute=self.alarm_minute, second=self.alarm_second)
        # 每隔2分钟发送一条数据用于测试。
        ...
        for girlfriend in self.girlfriend_list:
            ...
            if not is_test:
                if self.is_online(auto_login=True):
                    # 将信息分成多条发送
                    for words in today_msg:
                        itchat.send(words, toUserName=name_uuid)
                        time.sleep(2)
                # 防止信息发送过快。
                time.sleep(5)
    ...
    def get_weather_info(self, dictum_msg='', city_code='101030100', start_date='2019-05-01', sweet_words='From your Valentine'):
        '''
        ...
        if resp.status_code == 200 and self.isJson(resp) and resp.json().get('status') == 200:
            ...
            if start_date:
                ...
                delta_msg = f'宝贝儿这是我们在一起的第 {day_delta} 天。\n' # 没有儿化音的宝贝儿是没有灵魂的
                today_msg.append(delta_msg)
            
            today_msg += [f'下面是今日份的男票天气预报～\n{temperature}\n{wind}\n{aqi}\n{notice}', f'{dictum_msg}']

            if sweet_words:
                today_msg.append(sweet_words)
            return today_msg
